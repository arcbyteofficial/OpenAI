...
describe("Sidebar — collapse toggle stays RTL-safe", () => {
  const code = src("src/shared/components/Sidebar.tsx");

  it("pushes the collapse toggle to the row end with a growing brand link, not a margin", () => {
    // The brand link is flex-1, so the toggle lands at the inline end in LTR and RTL alike;
    // its inset is the logical ps-2.
    expect(code).toContain('"flex-1 gap-2.5 ps-2"');
  });
